...
public class ReservationRepositoryImpl {
    ...
    public List<findReDto> findReservations(int memberId) {
        List<findReDto> findDto = query.select(Projections.constructor(findReDto.class, qReservation.re_id,qReservation.create_date, qReservation.people, qReservation.request, qRestaurant.rs_name))
                ...
                .where(qReservation.mId.eq(memberId).and(qReservation.reservation_status.eq(1))).orderBy(qReservation.create_date.desc()).fetch();
        return findDto;
    }
    ...
    public Image findImage(Restaurant rsId){
        Image image = query.select(qImage).from(qImage).where(qImage.rs_id.eq(rsId).and(qImage.img_role.eq("1"))).fetchOne();
        return image;
    }
